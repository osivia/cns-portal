        ...
        <ul class="${fixed}">
            <c:forEach var="userPage" items="${userPages}">
                <c:if test="${not userPage.defaultPage}">
	                <c:if test="${!requestScope['osivia.user.administrator'] || userPage.defaultPage || (userPage.id eq requestScope['osivia.currentPageId']) || (not empty userPage.closePageUrl)}">
	                    <li role="presentation"
